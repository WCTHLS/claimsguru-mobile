...
export function inferGenderFromName(name?: string | null): 'male' | 'female' {
  if (!name) return 'male';
...
export const UserAvatar: React.FC<UserAvatarProps> = ({
  ...
  const effectiveGender = (
    baseGender ? baseGender : inferGenderFromName(name)
  ).toLowerCase().trim();

  const isFemale = effectiveGender === 'female' || effectiveGender === 'f';
  const ringColor = isFemale ? 'rgba(244, 114, 182, 0.6)' : 'rgba(56, 189, 248, 0.6)';
    ...
    <View
      style={[
        styles.container,
        {
          ...
          borderWidth: showRing ? Math.max(1.5, pixelSize * 0.04) : 0,
    ...
    >
      {isFemale ? (
        ...
        >
          ...
          {/* Female Back Hair: Shoulder-Length Flared Jagged Layers */}
          <Path
          ...
          {/* Almond Eyes with Pupils & Catchlights */}
          ...
          {/* Diagonal Side-Swept Bangs across forehead */}
          <Path
        ...
        <Svg
          ...
          height={pixelSize}
        >
          {/* Soft Sky Blue Circular Background */}
          ...
          {/* Male Hair Style: Tapered Sides, Clean Brow Hairline, Crested Top Spikes */}
          <Path
